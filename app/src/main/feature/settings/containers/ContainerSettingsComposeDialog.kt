...
class ContainerSettingsComposeDialog @JvmOverloads constructor(
    ...
    private fun loadResourceArrays() {
        ...
        state.gamescopeContainer.value = c?.isGamescopeRuntime == true
        loadLinuxProtons()
    ...
    private fun saveSettings() {
        ...
        if (c != null) {
            ...
            linuxProtonIds?.let { ids ->
                c.setLinuxProton(ids.getOrElse(state.selectedLinuxProton.intValue) { Container.LINUX_PROTON_DEFAULT })
            }
    ...
    private fun loadLinuxProtons() {
        linuxProtonIds = null
        state.linuxProtonEntries.value = emptyList()
        val c = container?.takeIf { it.isGamescopeRuntime } ?: return
        val saved = c.getLinuxProton()
        scope.launch {
            val choices = withContext(Dispatchers.IO) { LinuxProtons.choices(context) }
