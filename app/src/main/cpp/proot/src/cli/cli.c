...
static void print_execve_help(const Tracee *tracee, const char *argv0,
  ...
  note(tracee, INFO, USER,
       "possible causes:\n"
       "  * the program is a script but its interpreter (eg. /bin/sh) was not "
       "found;\n"
       "  * the program is an ELF but its interpreter (eg. ld-linux.so) was "
       "not found;\n"
...
static void print_error_separator(const Tracee *tracee,
  ...
  if (argument->separator == '\0')
    ...
    note(tracee, ERROR, USER,
         "option '%s' and its value must be separated by '%c'.", argument->name,
         argument->separator);
}
 ...
 */
int parse_integer_option(const Tracee *tracee, int *variable, const char *value,
                         const char *option) {
  char *end_ptr = NULL;
